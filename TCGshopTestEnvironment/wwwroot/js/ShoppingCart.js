$(document).ready(function() {
    shoppingCart.forEach(function(e) {
        ...
        var image = "<div class=\"shoppingCartImg\"><img src=\"" +
            storagePath +
            e.ImageUrl +
            ".png\" class=\"imageCell\" /></div>";
        $("#my-ajax-table > tbody:last-child").append('<div class="container" id="' +
            "row".concat(e.ProductId) +
            '">' +
            '<div class="row lineCart"><div class="col-md-2 imageShoppingJs"> ' +
            image +
            "</div>" +
            ...
            e.Name +
            "</h2> Amount: " +
            '<button class="btn btn-default btn-minPlus" id="' +
            e.ProductId +
            '">-</button> ' +
            '<input class="ShoppingQuant text-center" id="' +
            "input".concat(e.ProductId) +
            '" type="number" placeholder="' +
            e.Amount +
            '" value="' +
            e.Amount +
            '" min="1" oninput="this.value = Math.abs(this.value)" onchange="inputvalidatewithstock(' +
            e.ProductId +
            ')">' +
            '<button class="btn btn-default btn-minPlus" id="' +
            e.ProductId +
            '">+</button></div>' +
            '<div class="col-md-6 priceCart" style="font-size:2rem;" id="' +
            e.ProductId +
            '">Price: € ' +
            e.Price +
            ' p/u<div id = "RemoveItem"><h2 id="' +
            "totalprice".concat(e.ProductId) +
            '">€ ' +
        ...
        $("#overzicht > tbody:last-child").append(
            '<div class="container-fluid" style="background-color: #313337;" id="' +
            "overzichtrow".concat(e.ProductId) +
            '">' +
            '<div class="row lineCartInformation"><div class="col-md-7"><p class="ShoppingCartTitleInfo">' +
            e.Name +
            "</p></div>" +
            '<div class="col-md-1" id="' +
            "overzicht".concat(e.ProductId) +
            '">x' +
            e.Amount +
            "</div>" +
            '<div class="col-md-4 priceCartInfo text-right" style="font-size:2rem;" id="' +
            "totaleprice".concat(e.ProductId) +
            '">€ ' +
            totalprice.toFixed(2) +
            ' <div id = "RemoveItem">' +
            "</div></div></div>");
